---
name: setup
description: Driver-controllable F1 setup advisory — brake bias, differential, ERS deploy mode, fuel mix, DRS timing, wing damage. Suggests cockpit tweaks vs garage notes.
when_to_use: Trigger event_code in {HardBraking, BrakeTemp, TireTemp, ERSLow, FuelLow, lap_complete}. Also any time you suspect a setting is mismatched to the driver's recent behaviour.
tools: ["get_state_driver_settings", "get_drs_usage", "get_state_tires", "get_state_energy", "get_state_competitors", "get_brake_balance_report", "get_corner_brake_history", "recent_pi_observations", "write_observation", "push_insight"]
---
...
- `front_wing_damage_pct_max > 25` AND ideal pit lap is still ahead →
  `write_observation topic=setup.wing_damage` with "front wing %d%% —
...
- Always reference the **current value** in the radio call. "BB 56 — try
  ...
  than the delta.
- Use `write_observation` (not `push_insight`) for advisory-only nudges
...
- Check `recent_pi_observations` first; don't repeat a recommendation
