...
| `PTT_BUTTON` | `0x00000001` | Steering wheel/controller button bitmask for push-to-talk |

See `.env.example` for the full list with descriptions.

## Push-to-Talk

Set `PTT_BUTTON` to the `BUTN` mask emitted by your controller or wheel. The observed PS5 stream uses `0x00000001`; mapped **UDP Action 1** usually uses `0x00100000`. When pressed during gameplay, the dashboard records audio from your microphone, transcribes it with Whisper, sends the query to the LLM with full telemetry context, and plays back the response as synthesized speech.
