---
name: tires
description: Tire wear, compound choice, cliff detection — trigger on Tire* event codes or every lap.
---
...
Only push when:
1. The signal is new (not in `recent_analyst_observations` last 5 min).
2. The driver can act on it within 1-2 laps.
3. The number is concrete ("rear wear at 78%, deg now 0.4 s/lap").

Otherwise just (write to `learnings/`) with `topic=tires` so other ticks see it.
