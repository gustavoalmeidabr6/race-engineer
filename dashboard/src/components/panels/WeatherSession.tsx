...
export function WeatherSession({ data }: Props) {
  ...
  // Track race events. BUTN events are intentionally skipped — controller
  // button state belongs to the Gemini Live / Go PTT pipeline, not the UI.
  useEffect(() => {
    if (!data.last_event_code || data.last_event_code === prevEventRef.current) return;
    prevEventRef.current = data.last_event_code;
    if (data.last_event_code === 'BUTN') return;
