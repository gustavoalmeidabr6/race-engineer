...
// rule families (closing_threat + car_approaching for the same car), and
...
// together. Currently: closing_threat + car_approaching. Lap_summary,
// pit_window etc. are kept separate because their messages don't
// naturally bundle.
func isThreatClass(t EventType) bool {
	switch t {
	case EventThreatOvertake, EventCarApproaching:
		return true
	default:
		return false
	}
...
func (b *RaceBrain) EnqueueEvent(e Event, talkLevel int32) (*Event, error) {
	...
	// Charles + car_approaching for Charles within the same window collapse
...
// AckDelivered marks the in_flight event as successfully spoken: removes it
// from the queue and records the spoken text into the L4 speech buffer for
// dedup-against-recent-radio. spokenText may differ from the event's summary
// (the dispatcher's TTS rephrases) — store whatever the dispatcher reports.
//
...
func (b *RaceBrain) AckDelivered(eventID, spokenText string, priority int) error {
	...
	for i, e := range b.eventQueue {
		...
		// Remove from queue.
		b.eventQueue = append(b.eventQueue[:i], b.eventQueue[i+1:]...)
		// Record speech inline (we already hold the lock — call the
		// underlying field directly to avoid a re-entrant lock).
		text := spokenText
		if text == "" {
			text = e.Summary
		}
...
// RunReaper runs ReapStaleLeases on a ticker until ctx is cancelled. Call
// once from main on startup; one goroutine is enough.
...
// hold b.mu. In_flight events are exempt — they may be slow to deliver but
// shouldn't be silently dropped from under the dispatcher; the lease reaper
// handles those instead.
func filterStaleLocked(q []Event, now time.Time) []Event {
	out := q[:0]
	for _, e := range q {
		if e.Status != StatusInFlight && e.StaleAfter > 0 && now.Sub(e.At) > e.StaleAfter {
			continue
		}
		out = append(out, e)
	}
