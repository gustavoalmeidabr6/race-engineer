...
// ---------------------------------------------------------------------------
// TestMockIngestion — verifies the full mock generator emits every packet
// type the writer cares about and that each one lands in DuckDB.
// ---------------------------------------------------------------------------
...
func TestMockIngestion(t *testing.T) {
	...
	// session and history tickers run at 0.5Hz / 0.2Hz, so give them a window.
	time.Sleep(3 * time.Second)
	cancel()
	wg.Wait()

	// Every packet type the mock generator emits should land in DuckDB.
	for _, tbl := range []string{
		"telemetry",         // CarTelemetry (20Hz)
		"car_telemetry_ext", // CarTelemetry (20Hz)
		"motion_data",       // Motion (20Hz)
		"lap_data",          // LapData (10Hz)
		"car_status",        // CarStatus (10Hz)
		"car_damage",        // CarDamage (2Hz)
		"session_data",      // Session (0.5Hz)
		"race_events",       // Lights-out + periodic event ticks
	} {
		assertTableHasRows(t, store, tbl)
	}

	// Spot-check telemetry sanity: mock player speed should be a believable km/h.
	var speed float64
	if err := store.Writer().QueryRow("SELECT speed FROM telemetry ORDER BY timestamp DESC LIMIT 1").Scan(&speed); err != nil {
		t.Fatal(err)
	}
	if speed < 60 || speed > 340 {
		t.Errorf("expected mock speed in [60,340], got %.1f", speed)
	}
