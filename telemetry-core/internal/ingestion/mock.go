...
type MockGenerator struct {
	...
	// Session fastest lap (across all cars) — used to gate FTLP packet
	// emission so the mock only fires the event when a lap is genuinely
	// faster than anything seen this session, the same way the real game
	// behaves. 0 means "no lap completed yet".
	sessionBestLapMs uint32

	// pendingFTLP is set inside stepPhysics when a new session-best lap
	// is detected; consumed by the Run loop on the next tick to emit a
	// single FTLP event packet. pendingFTLPVehicle carries the car index.
	pendingFTLP        bool
	pendingFTLPVehicle uint8
}
...
func (mg *MockGenerator) Run(ctx context.Context, cfg *config.Config, packetChan chan<- []byte, packetsRx *atomic.Uint64, mockEnabled func() bool) {
	...
	for {
		...
		case <-tick20Hz.C:
			...
			if mg.pendingFTLP {
				mg.emit(packetChan, packetsRx, mg.buildEvent("FTLP", mg.pendingFTLPVehicle))
				mg.pendingFTLP = false
			}
...
func (mg *MockGenerator) stepCar(idx int, dt time.Duration) {
	...
	if c.distance >= float64ToF32(trackLength) {
		...
		// Session-best detection: only the player car's lap times beat
		// the session best in practice (the AI grid is static here), but
		// any car is eligible. Skip pit-affected laps — those carry the
		// 25 s penalty added above and aren't real fastest-lap candidates.
		if !c.pitting && lapTimeMs > 0 && (mg.sessionBestLapMs == 0 || lapTimeMs < mg.sessionBestLapMs) {
			mg.sessionBestLapMs = lapTimeMs
			mg.pendingFTLP = true
			mg.pendingFTLPVehicle = uint8(idx)
		}
...
func (mg *MockGenerator) maybeEvent() []byte {
	// FTLP is driven by real lap completion in stepCar; the periodic
	// ticker now only fires the occasional AI overtake event so the
	// dashboard / event bus see *something* happening mid-race without
	// being flooded.
	if rand.Intn(100) < 15 {
		// Overtake between two random AI cars (~1 OVTK every ~5 min on
		// a 45 s ticker at 15% probability).
