...
// fastest lap, etc.). The callback runs on the state-writer goroutine so it
// must not block — push to a channel or update an in-memory store and return.
type EventHook func(code, detail string, vehicleIdx int)
...
// SetEventHook registers an optional callback fired for every race event.
// Must be called before Start. Pass nil to clear.
func (ing *Ingester) SetEventHook(h EventHook) {
	ing.eventHook = h
}
...
func (ing *Ingester) Start(ctx context.Context, wg *sync.WaitGroup) {
	...
	go selfHeal(ctx, wg, "state-writer", func(c context.Context) {
		stateWriter(c, ing.parsedChan, ing.store, ing.cfg.SampleRate, ing.cfg.HifreqSampleRate, ing.cfg, ing.PTTChan, ing.eventHook)
	})
...
func (ing *Ingester) udpLoop(ctx context.Context) {
	...
	lc := net.ListenConfig{
		Control: func(network, address string, c syscall.RawConn) error {
			return c.Control(func(fd uintptr) {
				...
				if udpMode == "broadcast" {
					_ = syscall.SetsockoptInt(handle, syscall.SOL_SOCKET, syscall.SO_BROADCAST, 1)
				}
			})
		},
	}
	...
	for {
		...
		default:
			drops := PacketChanDrops.Add(1)
			log.Warn().Uint64("total_drops", drops).Msg("Packet channel full, dropping UDP packet")
		}
	}
}
