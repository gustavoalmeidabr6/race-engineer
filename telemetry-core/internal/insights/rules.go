	...
	return Rule{
		...
		Evaluate: func(state *models.RaceState, cd *CooldownManager) *models.DrivingInsight {
			if state.CurrentLap > cd.LastLap() {
				...
				// Replaced: lap-summary is now a typed Event emitted via the
				// Interrupts Bus (see Engine.emitLapSummary). Returning nil
				// keeps the cooldown side-effects (ResetForNewLap) without
				// the old "Lap N started" filler line on every lap.
			}
			return nil
		},
	}
