...
// SetTranscript wires the per-session transcript hub. Optional — Hub
// works without it; the Live Debug tab simply won't see WS lifecycle
// rows when nil.
func (h *Hub) SetTranscript(t *transcript.Hub) {
	h.mu.Lock()
	h.transcript = t
	h.mu.Unlock()
}
...
func (h *Hub) register(c *Client) {
	...
	t := h.transcript
	h.mu.Unlock()
	log.Info().Int("clients", n).Msg("WebSocket client connected")
	if t != nil {
		remote := ""
		if c.conn != nil {
			remote = c.conn.RemoteAddr().String()
		}
		t.Append(transcript.Event{
			Kind:  transcript.KindWSConnected,
			Actor: "ws_hub",
			Text:  "client connected",
			Meta:  map[string]any{"remote": remote, "clients": n, "route": "/ws"},
		})
	}
}
...
func (h *Hub) unregister(c *Client) {
	...
	t := h.transcript
	h.mu.Unlock()
	log.Info().Int("clients", n).Msg("WebSocket client disconnected")
	if t != nil {
		remote := ""
		if c.conn != nil {
			remote = c.conn.RemoteAddr().String()
		...
		t.Append(transcript.Event{
			Kind:  transcript.KindWSDisconnected,
			Actor: "ws_hub",
